...
Commands.prototype._replaceInsert = function(cmd, reqId, spaceId, tuple){
  ...
  Commands.prototype._auth = function(username, password){
	...
	return new Promise(function (resolve, reject) {
		...
		_this.socket.send(toArrayBuffer(buffer));
	});
};
...
function scramble(password, salt){
	...
	var step3 = shatransform(Buffer.concat([new Buffer(encSalt.slice(0, 20)), new Buffer(convertWordArrayToUint8Array(step2))]).toString('hex'));
	return xor(convertWordArrayToUint8Array(step1), convertWordArrayToUint8Array(step3));
}
